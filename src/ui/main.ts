...
function renderChainResult(analysis: Extract<TransactionAnalysisResult,{kind:'chain'}>): string {
  ...
  if (payload.status === 'blocked') { const moving = payload.chain.movingSupply; const detail = 'rationale' in moving ? moving.rationale : moving.issues.join(' · '); return `<div class="result"><h3>Lieferkette ${badge('blocked')}</h3><p class="muted">${esc(detail)}</p></div>`; }
